...
async function getRecentProjects() {
  ...
  const recentProjects = repos
    .filter(repo => !repo.fork && !repo.archived && repo.name !== 'sychox2006')
    .slice(0, 3)
    .map(repo => `- [${repo.name}](${repo.html_url}) - ${repo.description || 'No description'}`);
...
async function updateReadme() {
  ...
  const mostActiveRepoRegex = /<a href="https:\/\/github\.com\/sychoxhassan\/.*">\s*<img src="https:\/\/github-readme-stats\.vercel\.app\/api\/pin\/\?username=sychoxhassan&repo=.*&theme=radical" alt="Most Active Repository" \/>\s*<\/a>/;
  const newMostActiveRepoContent = `<a href="${mostActiveRepo.html_url}">\n    <img src="https://github-readme-stats.vercel.app/api/pin/?username=sychoxhassan&repo=${mostActiveRepo.name}&theme=radical" alt="Most Active Repository" />\n  </a>`;
